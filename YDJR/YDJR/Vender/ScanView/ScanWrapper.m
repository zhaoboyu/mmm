...
//ios7之后native封装
@property(nonatomic,strong)ScanNative* scanNativeObj;
...
- (instancetype)initWithPreView:(UIView*)preView ArrayObjectType:(NSArray*)arrayBarCodeType cropRect:(CGRect)cropRect
                        success:(void(^)(NSArray<ScanResult*> *array))blockScanResult
{
    if (self = [super init])
    {
//        self.isUseZXingLib = NO;
        ...
        if ( [ScanWrapper isSysIos7Later] )
        {
            _scanNativeObj = [[ScanNative alloc]initWithPreView:preView ObjectType:arrayBarCodeType cropRect:cropRect success:^(NSArray<ScanResult*> *array) {
...
//    if ( [ScanWrapper isSysIos7Later] && !_isUseZXingLib )
...
+ (UIImage*)createQRWithString:(NSString*)str size:(CGSize)size
{
    if ([ScanWrapper isSysIos7Later]) {
        ...
        return [ScanWrapper createNonInterpolatedUIImageFormCIImage:[ScanWrapper createQRForString:str] withSize:size.width];
    }else{
        return nil;
    }
//    else
